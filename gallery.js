// gallery.js - Versión FINAL corregida (miniatures visibles + carga garantizada al canvas)
console.log("✅ gallery.js v6 - Carga directa al canvas");
...
window.addEventListener('load', function() {
  ...
  if (closeGalleryBtn) closeGalleryBtn.addEventListener('click', () => galleryOverlay.style.display = 'none');
});

async function cargarGaleriaDesdeSheets() {
  const grid = document.getElementById('imgbb-designs-grid');
  grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px;color:#64748b;font-family:monospace;">Cargando miniaturas...</div>`;
...
function filterAndRender(busqueda = '') {
  ...
  filtrados.forEach(d => {
    ...
    imgEl.onerror = () => { imgEl.style.opacity = '1'; imgEl.src = 'https://via.placeholder.com/140/1e2937/64748b?text=Error'; };

    grid.appendChild(item);
  });

  if (filtrados.length === 0) grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;color:#64748b;padding:60px;">No se encontraron diseños</div>`;
}
...
function loadSingleDesign(url) {
  ...
  img.onload = () => {
    ...
    designs.push(design);
    window.designs = designs;           // ← Forzamos global
    selectedDesignId = id;

    updateDesignsList();
    updateControls();
    window.drawCanvas();                // ← Llamada directa global
